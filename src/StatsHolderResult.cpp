/*Стоило наверное делать абстрактный класс или как-то по другому классы реализовать
убрав этому классу возможность довалять по пакету
И отдельный фаил для вложенной структуры
И Нормальный конструктор для вложенной структуры через HTTP пакет а не руками,
но там падает при попытке изнутри функции даже по копии взять взять поле хоста
оффсет инвалидируется какой-то
*/
